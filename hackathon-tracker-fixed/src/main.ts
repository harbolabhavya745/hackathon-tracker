...
async function init() {
  ...
  // 1. Get initial session first (synchronously before subscribing)
  try {
  ...
  // 2. Listen for future auth changes (sign in / sign out events only)
  authApi.onAuthStateChange(async (event, session) => {
    // INITIAL_SESSION fires on every load — we already handled it above, skip it
    ...
    if (state.user && state.user.id !== prevUserId) {
      // New user signed in — reset all state and reload
      state.hackathons = [];
      state.currentHackathonId = null;
      state.cache = { members: {}, dates: {}, tasks: {}, regs: {} };
      if (activeChannel) {
      ...
      // User signed out — tear down subscription
      if (activeChannel) {
        activeChannel.unsubscribe();
        activeChannel = null;
      }
